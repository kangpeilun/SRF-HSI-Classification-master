...
# setting parameters
# DataPath = '../../data/other_models_data/IndinePine2011.mat'
# TRPath = '../../data/other_models_data/IndinePines_Tr.mat'
# TSPath = '../../data/other_models_data/IndinePines_Te.mat'

DataPath = '../../data/IndianPine.mat'
# TRPath = '../../data/IndinePines_Tr.mat'
# TSPath = '../../data/IndinePines_Te.mat'

savepath = './saved_model/2DCNN-result.mat'

patchsize = 1  # input spatial size for 2D-CNN
...
# Data = File(DataPath, 'r')
# TrLabel = io.loadmat(TRPath)
# TrLabel = File(TRPath, 'r')
# TsLabel = io.loadmat(TSPath)
# TsLabel = File(TSPath, 'r')
...
TrainNum = len(ind1)  # 695
TrainPatch = np.empty((TrainNum, l, patchsize, patchsize), dtype='float32')  # [695, 200, 16, 16]
TrainLabel = np.empty(TrainNum)  # 695
ind3 = ind1 + pad_width     # de dao tuo zhan hou xin zuo biao
ind4 = ind2 + pad_width
for i in range(len(ind1)):
    if patchsize % 2 != 0:   # patchsize奇数
    ...
    patch = np.reshape(patch, (patchsize * patchsize, l))   # [256, 200]
    patch = np.transpose(patch)  # [200, 256]
    patch = np.reshape(patch, (l, patchsize, patchsize))    # [200, 16, 16]
    TrainPatch[i, :, :, :] = patch
    patchlabel = TrLabel[ind1[i], ind2[i]]
    TrainLabel[i] = patchlabel

[ind1, ind2] = np.where(TsLabel != 0)
TestNum = len(ind1)  # 9671
...
print('Training size and testing size are:', TrainPatch.shape, 'and', TestPatch.shape)  # (695, 200, 16, 16) and (9671, 200, 16, 16)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(OutChannel, OutChannel*2, 1, 1, 1),
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(OutChannel*2, OutChannel*4, 1, 1, 1),
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
        ...
        if step % 50 == 0:
            ...
            if accuracy > BestAcc:
                torch.save(cnn.state_dict(), './saved_model/2DCNN.pth')
                BestAcc = accuracy
...
cnn.load_state_dict(torch.load('./saved_model/2DCNN.pth'))
cnn.eval()
